...
try {
    ...
    res.json(dealers); // Return the dealerships in JSON format
  } catch (error) {
...
//Write your code here
try {
    const stateDealers = await Dealerships.find({ state: req.params.state }); // Fetch dealers based on state
    res.json(stateDealers); // Return the state-specific dealerships in JSON format
  } catch (error) {
